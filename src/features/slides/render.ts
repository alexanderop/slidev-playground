...
import { BUILTIN_SLIDE_COMPONENTS } from './builtin-components'
...
export function renderSlidesOnce(inputs: RenderInputs): readonly RenderedSlide[] {
  return createRenderer().render(inputs)
}
...
export function useSlideRenderer(inputs: {
  ...
  const renderer = shallowRef(createRenderer())
  watch(
    () => toValue(inputs.customComponents),
    () => {
  ...
  return computed(() =>
    renderer.value.render({
      slides: toValue(inputs.slides),
      config: toValue(inputs.config),
      defaults: toValue(inputs.defaults),
      customComponents: toValue(inputs.customComponents),
    }),
  )
}

type Renderer = {
  render: (inputs: RenderInputs) => readonly RenderedSlide[]
}

function createRenderer(): Renderer {
  const compiledComponentCache = new Map<string, Component>()
  const katexState: KatexPluginResult = { mathClicks: 0 }
  ...
  function renderMarkdown(content: string, options: RenderMarkdownOptions): RenderMarkdownResult {
    ...
    codeClicks = Math.max(codeClicks, katexState.mathClicks)
  ...
  function renderSlide(
    ...
    const perSlot: { directiveClicks: number; markdownClicks: number }[] = []
    let scopedStyles = ''
    ...
    for (const slot of slots) {
      ...
      perSlot.push({
        directiveClicks: totalClicks,
        markdownClicks: rendered.codeClicks,
      })
      if (styles.length > 0) {
  ...
  return {
    render({ slides, config, defaults, customComponents }) {
      return slides.map((slide, index) =>
        renderSlide(slide, index, config, defaults, customComponents),
      )
    },
  }
}
...
function reconcileSlideClicks(
  perSlot: readonly { directiveClicks: number; markdownClicks: number }[],
): number {
  let result = 0
  for (const slot of perSlot) {
    result = Math.max(result, slot.directiveClicks, slot.markdownClicks)
  }
  return result
}
